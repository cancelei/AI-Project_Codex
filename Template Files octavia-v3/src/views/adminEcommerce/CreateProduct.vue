<script setup>
import { crmOne, crmTwo, tasks } from "@/data/crm.js";
import ProductGridCard from "@/components/ProductGridCard.vue";
</script>

<template>
  <v-container>
    <v-row>
      <v-col cols="12" class="product-list-card px-0">
        <div
          class="d-flex justify-space-between align-center mb-4 bb-secondary pb-3 px-4 flex-wrap"
        >
          <div class="d-flex align-center mb-3">
            <div class="bg-primary-lighten-5 px-3 py-2 rounded me-3">
              <i class="tio- text-primary text-18">shopping</i>
            </div>
            <div class="text-subtitle-2 f-600">Iphone</div>
          </div>
          <div class="d-flex mb-3">
            <router-link
              to="#"
              class="text-decoration-none product-link active me-8 f-600"
              >Active</router-link
            >
            <router-link
              to="#"
              class="text-decoration-none product-link me-8 f-600"
              >Draft</router-link
            >
            <router-link
              to="#"
              class="text-decoration-none product-link me-8 f-600"
              >Assembly</router-link
            >
          </div>
          <v-btn color="primary" flat class="text-capitalize">
            <i class="tio- me-2">add</i>
            Save Product</v-btn
          >
        </div>
        <v-row class="px-4">
          <v-col cols="12" lg="6">
            <base-card>
              <div class="text-subtitle f-600 mb-4">Main Parameters</div>
              <v-row>
                <v-col cols="12" lg="6">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Manufacturer"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="6">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Model"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="6">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="ID Number"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="6">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Priority"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="6">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Name"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="6">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Model"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Meta Title"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Meta Tags"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Meta Descriptions"
                    hide-details
                  ></v-text-field>
                </v-col>
              </v-row>
            </base-card>
          </v-col>
          <v-col cols="12" lg="6">
            <base-card>
              <div class="text-subtitle f-600 mb-4">Prices and Warehouses</div>
              <v-row>
                <v-col cols="12" lg="4">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Cost"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="4">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Extra"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="4">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Price"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="4">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Availability"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="4">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Amount"
                    hide-details
                  ></v-text-field>
                </v-col>
                <v-col cols="12" lg="4">
                  <v-text-field
                    color="primary"
                    variant="outlined"
                    label="Name"
                    hide-details
                  ></v-text-field>
                </v-col>
              </v-row>
              <div class="text-subtitle f-600 mb-4">Description</div>
              <v-row>
                <v-col cols="12">
                  <v-textarea
                    hide-details
                    color="primary"
                    variant="outlined"
                    label="description ..."
                  ></v-textarea>
                </v-col>
              </v-row>
            </base-card>
          </v-col>
        </v-row>
      </v-col>
    </v-row>
  </v-container>
</template>

<style lang="scss" scoped>
.mw-200 {
  max-width: 200px;
}
.product-list-card {
  box-shadow: 0px 12px 50px 2px rgba(19, 80, 124, 0.14);
  border-radius: 8px;
}
.product-link {
  color: #647283;
  padding-bottom: 12px;
  border-bottom: 2px solid transparent;
  &.active {
    color: #0a68ff;
    border-bottom: 2px solid #0a68ff;
  }
}
</style>
